...
import uk.gov.gchq.palisade.contract.filteredresource.kafka.KafkaInitializer.Config;
...
class KafkaRestWebSocketContractTest {
    ...
    private static TriFunction<String, String, String, WebSocketMessage> getErrorBuilder() {
        return (token, serviceName, errorMessage) -> Builder.create()
    ...
    private static BiFunction<String, LeafResource, WebSocketMessage> getResponseBuilder() {
        return (token, leafResource) -> Builder.create()
    ...
    private <T> T deserialize(final String json, final Class<T> clazz) {
        try {
            ...
            return Stream.of(
                    ...
                    Arguments.of(
                            ...
                            List.of(
                                    ...
                                    Builder.create().withType(MessageType.COMPLETE).withHeader(Token.HEADER, "test-token-4").noHeaders().noBody()
                            ),
                            List.of(
